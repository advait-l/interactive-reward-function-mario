...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/mainGame/MarioGame.js":[function(require,module,exports) {
...
function MarioGame() {
  ...
  this.collisionCheck = function (objA, objB) {
    ...
    var vY = objA.y + objA.height / 2 - (objB.y + objB.height / 2); // add the half widths and half heights of the objects

    var hWidths = objA.width / 2 + objB.width / 2;
    var hHeights = objA.height / 2 + objB.height / 2;
    var collisionDirection = null; // if the x and y vector are less than the half width or half height, then we must be inside the object, causing a collision
  ...
  this.checkElementMarioCollision = function (element, row, column) {
    ...
    if (collisionDirection == 'l' || collisionDirection == 'r') {
      ...
      if (element.type == 3) {
        //PowerUp Box
        var powerUp = new PowerUp(); //gives mushroom if mario is small, otherwise gives flower
  ...
  this.checkPowerUpMarioCollision = function () {
    for (var i = 0; i < powerUps.length; i++) {
      ...
      if (collWithMario) {
        ...
        score.updateTotalScore(); //sound when mushroom appears
    ...
    for (var i = 0; i < goombas.length; i++) {
      if (!mario.invulnerable && goombas[i].state != 'dead' && goombas[i].state != 'deadFromBullet') {
        ...
        if (collWithMario == 't') {
          ...
          score.totalScore += 1000;
          score.updateTotalScore(); //sound when enemy dies
          ...
          if (mario.type == 'big') {
            ...
            collWithMario = undefined; //sound when mario powerDowns
            ...
            score.lifeCount--;
            score.updateLifeCount(); //sound when mario dies
  ...
  this.wallCollision = function () {
    ...
    if (mario.y >= height) {
      that.pauseGame(); //sound when mario dies
  ...
  this.updateMario = function () {
    ...
    if (keys[38] || keys[32]) {
      ...
      if (!mario.jumping && mario.grounded) {
        mario.jumping = true;
        mario.grounded = false;
        mario.velY = -(mario.speed / 2 + 5.5); // mario sprite position
    ...
    if (keys[17] && mario.type == 'fire') {
      ...
      if (!bulletFlag) {
        ...
        bullets.push(bullet); //bullet sound
    ...
    if (mario.grounded) {
      mario.velY = 0; //grounded sprite position
  ...
  this.checkMarioPos = function () {
    centerPos = translatedDist + viewPort / 2; //side scrolling as mario reaches center of the viewPort
  ...
  this.levelFinish = function (collisionDirection) {
    ...
    if (marioInGround) {
      ...
      if (tickCounter > maxTick) {
        ...
        mario.x += 10;
        tickCounter = 0;
        mario.frame = 12; //sound when stage clears
...
},{}]},{},["../../../../usr/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/mainGame/MarioGame.js"], null)
//# sourceMappingURL=/MarioGame.2dd50fa8.js.map